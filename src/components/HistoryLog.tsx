...
import { History, ArrowUpCircle, ArrowDownCircle, Circle } from 'lucide-react';

interface Props {
  history: HistoryEntry[];
  users: User[];
  /** Domyślnie "Historia postaci" - użyj np. "Historia postaci — Adam" w panelu osoby. */
  title?: string;
}

export const HistoryLog: React.FC<Props> = ({ history, users, title }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex items-center gap-3">
        ...
        <h2 className="text-lg font-semibold text-slate-800">{title || 'Historia postaci'}</h2>
      </div>

      <div className="max-h-[400px] overflow-y-auto divide-y divide-slate-100">
        {history.length === 0 ? (
          <div className="p-6 text-sm text-slate-400 italic text-center">
            Brak zdarzeń. Historia wypełnia się przy zmianach punktów, przypisaniach zadań, edycji ustawień i harmonogramu.
          </div>
        ) : (
          history.map(entry => {
