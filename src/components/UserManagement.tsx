...
import { Shield, ShieldAlert, User as UserIcon, Settings2, Trash2, Edit2, Plus, Bell, Link2 } from 'lucide-react';

interface Props {
  users: User[];
  /** Prawdziwy hass (dostępny tylko gdy panel działa w Home Assistant) - używany
   *  do wyszukania encji person.* i pokazania jej zdjęcia/imienia. */
...
export function UserManagement({ users, hass, onAddUser, onUpdateUser, onRemoveUser }: Props) {
  const [isAdding, setIsAdding] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);

  const [formData, setFormData] = useState<Omit<User, 'id'>>(emptyForm);

  // Prawdziwe osoby znane Home Assistantowi (person.*) - do powiązania z chore-userem.
  const personEntities = useMemo(() => {
    ...
    return Object.entries(hass.states)
      ...
      .map(([id, s]) => ({
        ...
        picture: s.attributes.entity_picture as string | undefined,
      }));
  }, [hass]);

  const getLinkedPerson = (user: User) =>
    user.personEntityId ? personEntities.find(p => p.id === user.personEntityId) : undefined;
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  onChange={e => setFormData(f => ({ ...f, personEntityId: e.target.value || undefined }))}
                  className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {users.map(user => {
            const linkedPerson = getLinkedPerson(user);
            return (
            <div key={user.id} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow">
              {linkedPerson?.picture ? (
                <img src={linkedPerson.picture} alt={user.name} className="w-12 h-12 rounded-full object-cover border border-slate-200 flex-shrink-0" />
              ) : (
              ...
              <div className="flex-1">
                ...
                <div className="text-sm font-mono text-slate-500 mb-1 flex items-center gap-1">
                  <UserIcon className="w-3 h-3" /> {user.haEntityId}
                </div>
                {linkedPerson && (
                  <div className="text-xs text-emerald-600 mb-2 flex items-center gap-1">
                    <Link2 className="w-3 h-3" /> Powiązano z {linkedPerson.id}
                  </div>
                )}
